     ...
     <div class="registration-form__form" x-data="registerData()">
           <form method="post">
             <div class="row registration-space-y">   
               ...
               <div class="col">
                 <input type="text" name="last_name" value="<?= set_value('last_name', $userInfo['last_name'])?>" class="form-control" placeholder="Last name" aria-describedby="last name">
                </div>
             </div><!-- /row -->
             <div class="row registration-space-y">   
               <div class="col">
                 <input type="text" name="father_name" value="<?= set_value('father_name', $userInfo['father_name'])?>" class="form-control" placeholder="Father name" aria-describedby="father name">
                </div>
               <div class="col d-flex align-items-center">
                   <div class="form-check" style="margin-right: 19px;">
                       <label class="form-check-label">
                       <input type="radio" class="form-check-input" name="sex" value="Male" <?= set_value('sex', $userInfo['sex']) == 'Male'? 'checked': '' ?> />
                        Male
                     </label>
                   </div>
                   <div class="form-check">
                       <label class="form-check-label">
                       <input type="radio" class="form-check-input" name="sex" value="Female" <?= set_value('sex', $userInfo['sex']) == 'Female'? 'checked': '' ?> />
                        Female
                     </label>
                   </div>
                </div><!-- /check-box-group -->
            ...
            <div class="row registration-space-y">   
                <div class="col">
                    <input type="text" name="email" value="<?= set_value('email', $userInfo['email']) ?>" <?= set_value('email', $userInfo['email']) == '' ? '' : 'disabled'  ?>  class="form-control" placeholder="Email" aria-describedby="email">
                    </div>
                <div class="col">
