...
<?php elseif(session()->get('success')): ?>
 ...
 <?php elseif(!$userInfo['is_info_confirmed']): ?>
    ...
    <div class="my-3 pt-3 box-alert warning-alert d-flex align-items-start" >
      ...
      <div class="message-alert"> 
          <h2 class="mb-2">Hello, <?= session()->get('first_name'); ?>. Your account is not activated  yet. </h2>
          <p> Please contact with your system admins to confirm your information. </p>
      </div>
   </div>
   <!-- WARNING AND ERROR AREA -->
 ...
 <!-- WARNING AND ERROR AREA -->
    ...
    <div class="registration-form__heading d-flex justify-content-between align-items-center mb-3">
         ...
         <?php if(!session()->get('isConfirmed')):?>
          <a href="/account/delete/<?= session()->get('id'); ?>" class="btn btn-outline-warning btn-rounded"> Delete Info </a>
         <?php endif; ?>

         <?php if(session()->get('isConfirmed')):?>
           <a href="/dashboard/overview" class="btn btn-outline-warning btn-rounded"> view dashboard </a>
         <?php endif; ?>
     ...
     <div class="registration-form__form">
         <form method="POST" >
            ...
            <div class="row registration-space-y">   
                <div class="col">
                    <input type="text" disabled name="email"  value="<?= set_value('email', $userInfo['email']); ?>" class="form-control" placeholder="Email" aria-describedby="email">
                    </div>
                <div class="col">
            ...
            <div class="registration-space-y">   
                  ...
                  <!-- <pre>
                     selected roles: <?= $userSelectedRole; ?>
                  </pre> -->
             ...
             <div class="registration-space-y">
                ...
                <div class="row"> 
                    <div class="col">
                      <input type="password" id="Inputpass" name="password" class=" form-control" placeholder="enter new password"  aria-describedby="password">
                    </div>
                    <div class="col">
                      <input type="password" name="password_confirm" class=" form-control" placeholder="confirm new password"  aria-describedby="confirm password">
                    </div>
                </div>
            ...
            <div class="row mt-3">
                 <div class="col">
                     <a href="<?= (session()->get('isLoggedIn')  && session()->get('isConfirmed'))? '/dashboard/overview' :'/logout' ?>" class="btn btn-warning btn-rounded"> Cancel </a>
                </div>
